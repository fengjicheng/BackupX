...
const BUILTIN_FIELD_CONFIG: Record<string, StorageTargetFieldConfig[]> = {
  local_disk: [
    ...
    {
      key: 'masterRelay',
      label: '远程备份经 Master 中转',
      type: 'switch',
      description:
        '开启后，远程 Agent 会把产物流式传给 Master 并写入上述目录；关闭则沿用 Agent 本机目录。',
    },
  ],
  s3: [
    {
...
const EXCLUDED_BACKENDS = new Set([
  ...
  'archive',
  'memory',
  'tardigrade', // tardigrade = storj 别名
  'union',
  'crypt',
  'chunker',
  'compress',
...
export function buildAllTypeOptions(
  rcloneBackends: { name: string; description: string }[],
): TypeOption[] {

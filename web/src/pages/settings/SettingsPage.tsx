  ...
  async function handleCheckUpdate() {
    ...
    try {
      ...
      setUpdateResult({
        ...
        latestVersion: '-',
        hasUpdate: false,
        error: resolveErrorMessage(e, '检查更新失败'),
      })
    } finally {
      setChecking(false)
    }
          ...
          <Card loading={loading} title="运行信息">
            <Descriptions
              ...
              data={[
                ...
                },
                {
                  ...
                  value:
                    info?.mode === 'release' ? (
                      ...
                      <Tag color="orange">{info?.mode ?? '-'}</Tag>
                    ),
                },
                ...
                {
                  label: '数据库路径',
                  value: <Typography.Text copyable>{info?.databasePath ?? '-'}</Typography.Text>,
                },
              ]}
            />
          </Card>
          ...
          <Card loading={loading} title="磁盘状态">
            <Descriptions
              ...
              data={[
                ...
                {
                  label: '使用率',
                  value: info?.diskTotal
                    ? `${((info.diskUsed / info.diskTotal) * 100).toFixed(1)}%`
                    : '-',
                },
              ]}
            />
            ...
            <Space direction="vertical" size="medium" style={{ width: '100%' }}>
              <Space>
                ...
                <Typography.Text type="secondary">
                  （当前：{updateResult.currentVersion}）
                </Typography.Text>
              </Space>
              {updateResult.publishedAt && (
                <Typography.Text type="secondary">
                  发布时间：{new Date(updateResult.publishedAt).toLocaleString()}
                </Typography.Text>
              )}
